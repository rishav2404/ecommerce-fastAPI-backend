...
def list_products(
    name: Optional[str] = Query(
        None, description="Filter by product name (supports partial matching)"
    ),
    size: Optional[str] = Query(None, description="Filter by available size"),
    limit: int = Query(
        10, ge=1, le=100, description="Maximum number of products to return"
    ),
    ...
    try:
        result = product_service.get_products(
            name=name, size=size, limit=limit, offset=offset
        )
